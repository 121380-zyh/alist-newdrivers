...
func (d *S3) Put(ctx context.Context, dstDir model.Obj, stream model.FileStreamer, up driver.UpdateProgress) error {
	uploader := s3manager.NewUploader(d.Session, func(u *s3manager.Uploader) {
		u.PartSize = 64 * 1024 * 1024 // 设置分块上传时每个块的大小为64MB
		u.Concurrency = 5             // 设置上传并发数为5，可以根据实际情况进行调整
	})
	...
	_, err := uploader.UploadWithContext(ctx, input, func(u *s3manager.Uploader) {
		u.LeavePartsOnError = true // 如果上传中发生错误，保留已上传的分块，下次继续上传
		u.MaxUploadParts = 10000   // 设置最大上传分块数，可以根据实际情况进行调整
	})
	return err
}
